...
class Book {
	...
	updateStorage = () => {
		if (JSON.parse(localStorage.library) !== myLibrary) {
	...
	render = (locationBefore) => {
		...
		card.innerHTML = `<h1> ${this.title} <h1>
...
const validateForm = () => {
	if (!/[a-z0-9]+/gi.test(document.forms['newBookForm'].title.value)) {
		...
		return false;
	} else if (!/[a-z ]{3,}/i.test(document.forms['newBookForm'].author.value)) {
		alert('Please give a valid author');
		return false;
	} else if (!/\d+/.test(document.forms['newBookForm'].pages.value)) {
		alert('Please provide pages as a number');
		return false;
	} else if (
		!(
			document.querySelector('#haveRead').checked ||
			document.querySelector('#haveNotRead').checked
		)
	) {
	...
	if (
		myLibrary.findIndex(
			(item) =>
				...
				document.forms['newBookForm'].title.value.toLowerCase()
		) >= 0
	) {
...
newBookSubmit.addEventListener('click', (e) => {
	...
	if (validateForm() && validateNotExisting()) {
		let newBook = new Book(
			document.forms['newBookForm'].title.value,
			document.forms['newBookForm'].author.value,
			document.forms['newBookForm'].pages.value,
			document.querySelector('#haveRead').checked
		);
		newBook.render();
		myLibrary.push(newBook);
